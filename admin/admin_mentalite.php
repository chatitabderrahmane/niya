...
if(isset($_POST['title']) AND isset($_POST['description'])){
    ...
    try{
        $rs = $con->prepare("INSERT INTO mentalite ( title, description ) VALUES ( ?, ? )");
...
if(isset($_GET['id'])){
    $id = $_GET['id'];
    try{
        $rs = $con->prepare("delete from mentalite where id = ?");
        $rs->execute([$id]);

        if($rs != 0 ){ 
            //echo "<script> alert('Deleted Successfully '); </script>";
            header('Location:./admin_mentalite.php');
        }else{
...
<tr>
<td colspan="2"><center><h1>Add Mentalite</h1></center></td>
</tr>
...
<table class="table table-dark table-borderless">
  ...
  <tbody class="table-light">
    ...
    <tr>
        ...
        <td><a href="./admin_mentalite.php?id=<?php echo $id; ?>" class="btn btn-danger">Delete</a></td>
    </tr>
